...
class networking_firewall_blocks_rdp_ingress_from_internet(Check):
    """Ensure Linode Firewalls block RDP ingress from the internet."""
        ...
        for fw in networking_client.firewalls:
            ...
            if firewall_allows_port_from_internet(fw, target_ports):
                ...
                report.status_extended = (
                    f"Firewall '{fw.label}' allows RDP (port(s) [3389]) "
                    f"ingress from the internet."
                )
            else:
                report.status = "PASS"
                report.status_extended = (
                    f"Firewall '{fw.label}' blocks RDP (port(s) [3389]) "
                    f"ingress from the internet."
                )
