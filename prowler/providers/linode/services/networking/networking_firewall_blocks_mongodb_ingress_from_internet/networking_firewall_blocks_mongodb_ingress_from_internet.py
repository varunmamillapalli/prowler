...
class networking_firewall_blocks_mongodb_ingress_from_internet(Check):
    """Ensure Linode Firewalls block MongoDB ingress from the internet."""

    def execute(self) -> list[CheckReportLinode]:
        findings = []
        target_ports = [27017, 27018]
        for fw in networking_client.firewalls:
            ...
            if firewall_allows_port_from_internet(fw, target_ports):
                ...
                report.status_extended = (
                    f"Firewall '{fw.label}' allows MongoDB (port(s) [27017, 27018]) "
                    f"ingress from the internet."
                )
            else:
                report.status = "PASS"
                report.status_extended = (
                    f"Firewall '{fw.label}' blocks MongoDB (port(s) [27017, 27018]) "
                    f"ingress from the internet."
                )
