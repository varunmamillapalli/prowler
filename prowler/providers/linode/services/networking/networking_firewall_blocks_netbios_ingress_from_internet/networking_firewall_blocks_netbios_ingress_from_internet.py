...
class networking_firewall_blocks_netbios_ingress_from_internet(Check):
    """Ensure Linode Firewalls block NetBIOS/SMB ingress from the internet."""

    def execute(self) -> list[CheckReportLinode]:
        findings = []
        target_ports = [137, 138, 139, 445]
        for fw in networking_client.firewalls:
            ...
            if firewall_allows_port_from_internet(fw, target_ports):
                ...
                report.status_extended = (
                    f"Firewall '{fw.label}' allows NetBIOS/SMB (port(s) [137, 138, 139, 445]) "
                    f"ingress from the internet."
                )
            else:
                report.status = "PASS"
                report.status_extended = (
                    f"Firewall '{fw.label}' blocks NetBIOS/SMB (port(s) [137, 138, 139, 445]) "
                    f"ingress from the internet."
                )
