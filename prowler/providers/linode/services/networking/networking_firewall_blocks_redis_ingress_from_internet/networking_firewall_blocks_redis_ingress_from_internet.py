...
class networking_firewall_blocks_redis_ingress_from_internet(Check):
    """Ensure Linode Firewalls block Redis ingress from the internet."""
        ...
        for fw in networking_client.firewalls:
            ...
            if firewall_allows_port_from_internet(fw, target_ports):
                ...
                report.status_extended = (
                    f"Firewall '{fw.label}' allows Redis (port(s) [6379]) "
                    f"ingress from the internet."
                )
            else:
                report.status = "PASS"
                report.status_extended = (
                    f"Firewall '{fw.label}' blocks Redis (port(s) [6379]) "
                    f"ingress from the internet."
                )
