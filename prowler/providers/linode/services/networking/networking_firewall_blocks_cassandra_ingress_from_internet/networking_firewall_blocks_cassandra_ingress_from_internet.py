...
class networking_firewall_blocks_cassandra_ingress_from_internet(Check):
    """Ensure Linode Firewalls block Cassandra ingress from the internet."""

    def execute(self) -> list[CheckReportLinode]:
        findings = []
        target_ports = [7199, 9160, 8888]
        for fw in networking_client.firewalls:
            ...
            if firewall_allows_port_from_internet(fw, target_ports):
                ...
                report.status_extended = (
                    f"Firewall '{fw.label}' allows Cassandra (port(s) [7199, 9160, 8888]) "
                    f"ingress from the internet."
                )
            else:
                report.status = "PASS"
                report.status_extended = (
                    f"Firewall '{fw.label}' blocks Cassandra (port(s) [7199, 9160, 8888]) "
                    f"ingress from the internet."
                )
