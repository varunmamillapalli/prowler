...
class networking_firewall_blocks_http_ingress_from_internet(Check):
    """Ensure Linode Firewalls block HTTP ingress from the internet."""
        ...
        for fw in networking_client.firewalls:
            ...
            if firewall_allows_port_from_internet(fw, target_ports):
                ...
                report.status_extended = (
                    f"Firewall '{fw.label}' allows HTTP (port(s) [80]) "
                    f"ingress from the internet."
                )
            else:
                report.status = "PASS"
                report.status_extended = (
                    f"Firewall '{fw.label}' blocks HTTP (port(s) [80]) "
                    f"ingress from the internet."
                )
