...
class networking_firewall_blocks_memcached_ingress_from_internet(Check):
    """Ensure Linode Firewalls block Memcached ingress from the internet."""
        ...
        for fw in networking_client.firewalls:
            ...
            if firewall_allows_port_from_internet(fw, target_ports):
                ...
                report.status_extended = (
                    f"Firewall '{fw.label}' allows Memcached (port(s) [11211]) "
                    f"ingress from the internet."
                )
            else:
                report.status = "PASS"
                report.status_extended = (
                    f"Firewall '{fw.label}' blocks Memcached (port(s) [11211]) "
                    f"ingress from the internet."
                )
